...
import me.ups3t.customcapes.Settings;
            ...
            if(requestMeta.startsWith("GET") && requestMeta.split(" ")[1].startsWith("/capes/")) {
                ...
                if (CapeUtils.getExemptUsers().contains(requestedUser)) {
                    if (CapeUtils.getUserCapeMap().containsKey(requestedUser)) {
                        ...
                        File requestedFile = new File(CapeUtils.CAPES_PATH + File.separator + CapeUtils.getUserCapeMap().get(requestedUser));
                ...
                } else if (!((boolean) Settings.getSettings().get("onlyRenderExemptUsers"))) {
